...
if ( bp_has_profile( 'profile_group_id=' . bp_get_current_profile_group_id() ) ) :
	...
	<?php
	$icon = '<i class="icon-large material-icons">settings</i>';
	$text=' Edit your ';
	if(bp_get_the_profile_group_name()=='Location'){
		$text = 'Choose your ';
		$icon = '<i class="icon-large material-icons">place</i>';
	}
	elseif(bp_get_the_profile_group_name()=='Profession'){
		$icon = '<i class="material-icons icon-large">assignment_ind</i>';
		$text = 'Choose your ';
	}
	?>
	<h3>
		<?php echo $icon.' '.$text.strtolower(bp_get_the_profile_group_name()); ?>
	</h3><form action="<?php bp_the_profile_group_edit_form_action(); ?>" method="post" id="profile-edit-form" class="standard-form copy-card <?php bp_the_profile_group_slug(); ?>">
...
<div style="clear:both">
		<input type="submit" class="btn btn-raised btn-success" name="profile-group-edit-submit" id="profile-group-edit-submit" value="<?php _e( 'Save Changes', 'buddypress' ); ?> " />
</div>
