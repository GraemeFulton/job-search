...
class WPUF_Subscription {
    ...
    function __construct() {
        add_action( 'wpuf_add_post_form_top', array($this, 'get_post_type') );

        add_filter( 'wpuf_add_post_args', array($this, 'set_pending'), 10, 2 );
        add_filter( 'wpuf_add_post_redirect', array($this, 'post_redirect'), 10, 3 );
     ...
     * @return string
     */
    function set_pending( $postdata, $form_id ) {

        //the default post form is set to new post, so we can check 
        $form_ids = wpuf_get_option( 'default_post_form', 'wpuf_general' );
        ...
        elseif ( wpuf_get_option( 'charge_posting', 'wpuf_payment' ) == 'yes' ) {
    ...
    /*
     * graeme additional function
     * if the post type is just a post, then it is equal to the form id of a post
     * 
     * if there is a match on these two, the post is free, as it is free to blog
     */
     ...
     * @return string
     */
    function post_redirect( $response, $post_id, $form_id ) {
        ...
        elseif ( self::has_user_error( $post_id ) ) {
