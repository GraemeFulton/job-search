...
// TODO May be better to check if they're not defined?
 ...
 * 
 */
function bp_like_users_who_like( $item_id = '' ) {
    $item_id = bp_get_activity_id();

    $users_who_like = array_keys( bp_activity_get_meta( $item_id , 'liked_count' , true ) );
    echo 'with array keys';
    print_r( $users_who_like );

    //echo 'without array keys';
    //  $users_who_like =  bp_activity_get_meta( $item_id , 'liked_count' , true );
    // print_r($users_who_like);
    // echo count($users_who_like);

    if ( count( $users_who_like ) == 1 ) {
        // TODO need check to see if this is user
        ?>
        <p><?php echo bp_core_get_userlink( $users_who_like , $no_anchor = false , $just_link = false ); ?><?php _e( ' liked this.' , 'buddypress-like' ); ?> </p>
        <?php
    } else {

        // need to change this for when 2 people like something...
        $others = count( $users_who_like ) - 1;
        echo bp_core_get_userlink( $users_who_like[$others] , $no_anchor = false , $just_link = false );
        _e( ' and ' , 'buddypress-like' );
        echo bp_core_get_userlink( $users_who_like[$others - 1] , $no_anchor = false , $just_link = false );
        _e( ' and ' , 'buddypress-like' );
        echo $others . _e( 'liked this' , 'buddypress-like' );
    }
}
    ...
    if ( $type == 'activity' ) :
        ...
        if ( is_user_logged_in() && $activity_type !== 'activity_liked' ) :
            ...
            if ( $users_who_like ) {
               ...
               // bp_like_get_likes( $bp_like_id , $type );
                ?>
 ...
 * bp_like_insert_head()
 *
 * Includes JavaScript needed in the <head>.
 *
 */
...
function bp_like_admin_page() {
    ...
    <div class="wrap">
        ...
        <form action="" method="post">
            ...
            <table class="form-table" id="bp-like-admin" style="max-width:650px;float:left;">
                <tr valign="top">
                    <th scope="row"><?php _e( 'General settings' , 'buddypress-like' ); ?></th>
                    <td>
                        <fieldset>
                            <legend class="screen-reader-text">
                                <span><?php _e( 'General settings' , 'buddypress-like' ); ?></span>
                            </legend>
    ...
    <?php _e( "Post an activity update when something is liked" , 'buddypress-like' ); ?>
                ...
                <tr valign="top">
                    <th scope="row"><?php _e( 'Privacy Settings' , 'buddypress-like' ); ?></th>
                    <td>
                        <fieldset>
                            <legend class="screen-reader-text">
                                <span><?php _e( "User Privacy" , "buddypress-like" ); ?></span>
                            </legend>
                            <p><?php _e( "Choose how much information about users who 'liked' a particular item is shown." , "buddypress-like" ); ?></p>
