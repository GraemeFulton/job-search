...
|--------------------------------------------------*/
// dtNode object
function dtNode(id, pid, name, url, title, slug, targ, rsspath){ 
	this.id = id;
	this.pid = pid;
	this.name = name;
	this.url = url;
...
wpdTree.prototype.a = function(id, pid, name, title,slug, path, link_target, rsspath){		
	if(rsspath != ""){
	...
	this.adtNodes[this.adtNodes.length] = new dtNode(id, pid, name, url, title,slug, link_target, rsspath); 
};
	...
	if(this.root.id != node.pid){
		if(node.url){
			...
			if(node.title) str += ' title="' + node.title + '"';
                        if(node.slug) str += ' slug="' + node.slug + '"';
			...
			if(true || node.title) str += ' title="' + node.title + '"';
                       if(true || node.slug) str += ' slug="' + node.slug + '"';
