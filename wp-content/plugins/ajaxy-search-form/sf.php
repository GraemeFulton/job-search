...
class AjaxyLiveSearch {
					...
					if($src = $this->get_image_from_content($post->post_content, $size['height'], $size['width'])){
						...
						if(in_array('{post_image_html}', $matches)) {
							$post_object->post_image_html = '<img src="'.$post_object->post_image.'" width="'.$src['width'].'" height="'.$src['height'].'" />';
                                            ...
                                            $img= get_the_image(56967);
                                            $post_object->post_image_html= '<img src="'.$img.'">';
				...
				if(strpos($match, 'custom_field_') !== false){
					...
					if ( is_array($custom_field) ) {
						...
						$post_object->{$cf_name} = apply_filters('sf_post_custom_field', $custom_field, $key, $post);
                         ...
                         $img= get_the_image($post->ID);
                                            $post_object->post_image_html= '<img src="'.$img.'" width="50" height="50" />';
	...
	{
            global $wp_query;
            $q= $wp_query->post->post_title;
            if(strpos($q,'Lostgrad')!==FALSE){$image='<img width="40px" style="margin-bottom:-15px;"src="http://lostgrad.com/wp-content/uploads/2014/01/admin-ajax1.png">';}else $image=='';
		$form = '
		...
		<div id="'.$settings['id'].'" class="sf_container">
			<form role="search" method="get" class="searchform" action="' . home_url( '/' ) . '" >
				<div>
					<label class="screen-reader-text" for="s">' . __('Search for:') . '</label>'.$image.'
					<div class="sf_search">
						<span class="sf_block">
							<input style="width:'.($settings['width']).'px;margin-left:0px;" class="sf_input" autocomplete="off" type="text" value="' . (get_search_query() == '' ? $settings['label'] : get_search_query()). '" name="s"/>
	...
	function selective_input($settings)
	{
		...
		<div id="'.$settings['id'].'" class="sf_ajaxy-selective-search" style="width:'.($settings['width']).'px;">
