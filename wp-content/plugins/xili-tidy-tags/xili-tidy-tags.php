...
define('XILITIDYTAGS_VER','1.9.0'); /* used in admin UI */
 ...
 *
 * Example running if jQuery is enable (use wp_head filter if not done by another plugin) to redirect to link of selected tag without a form
 *
 * @since 1.8.6
 *
 * @updated
 ...
 * plus
 * id_dropdown to set ID of <select> - by default 'xtt_cloud_drop' (will be used by jQuery) - define a unique if more than one dropdown
 *
 */
function xili_tidy_tags_dropdown ( $args ) {
	if ( is_array($args) )
		$r = &$args;
	else
		parse_str( $args, $r ); 
	...
	$defaults = array(
		...
		'exclude' => '', 'include' => '', 'link' => 'view', 'tagsgroup' => '', 'tagsallgroup' => '',
		'tidy_post_tag' => 'post_tag', 'echo' => false, 'id_dropdown' => 'xtt_cloud_drop',
	);
	$r = array_merge( $defaults, $r );
	$r['echo'] = false;
	...
	foreach ( $list as $one_line )  {
		...
		$dropdown .= '<option value="'.$one_line_link.'">'.$one_line_title.'</option>';
		
	}
	$dropdown .= '</select>
	<script type="text/javascript">
	//<![CDATA[
	jQuery("#'.$r['id_dropdown'].'").bind("change keyup",function()
 ...
 */
 ...
 //////
 function xtt_tags_from_group( $group_name, $mode = 'slug', $taxonomy, $taxonomy_child ) {
// from $group_name to ID;
$groupterm = term_exists($group_name, $taxonomy);
$group_id = $groupterm['term_id'];
//var_dump($group_id);
