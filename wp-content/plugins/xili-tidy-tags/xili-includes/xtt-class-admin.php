...
class xili_tidy_tags_admin extends xili_tidy_tags {
    ...
    {
        /* Register our script. */
        wp_register_script('datatables', plugins_url('js/jquery.dataTables.min.js', $this->file_file ) ); // 1.5.3.1 - min
        wp_register_style('table_style', plugins_url('css/xtt_table.css', $this->file_file ) );
	...
	function admin_enqueue_scripts() {
		wp_enqueue_script( 'datatables', plugins_url('js/jquery.dataTables.min.js', $this->file_file ) , array( 'jquery' ), '1.7.4', true );
	...
	function xili_add_tag_alias ( $tag ) {
		...
		$checkline = '<p>'. sprintf(__('This sub-form contains a field to define alias of this %1$s%2$s. Choose a %2$s already present in another language.','xili_tidy_tags'), $val, $this->singular_name) .'</p>';
		...
		$checkline .= '<select name="alias_of" id="alias_of">';
...
// dummy params 3.5 ;
		$terms = $wpdb->get_results($wpdb->prepare( $query, 'd' ) );
