...
function wpdt_get_taxonomy_nodelist($args){	
	...
	foreach ($taxonomyresults as $term){
            //gray: if taxonomy is location, we don't wanna see any tags that aren't in hierarchy order yet
            if($taxonomy=='location'&&!($term->parent==0 && ($term->slug!="united-kingdom"))){
        ...
        //gray: otherwise just revert to how it was before (without the extra if statement (location)
