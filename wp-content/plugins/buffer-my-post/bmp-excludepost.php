...
function bmp_exclude() {
    ...
    $response = null;
    $records_per_page = 20;
    $omit_cat=get_option('bmp_opt_omit_cats');
    ...
    if (isset($_POST["excludeall"])) {
        if (substr($_POST["delids"], 0, -1) == "") {
            print('
			<div id="message" style="margin-bmp:30px" class="updated fade">
				<p>' . __('No post selected please select a post to be excluded.', 'TweetOldPost') . '</p>
			</div>');
        } else {

            update_option('bmp_opt_excluded_post',$exposts);
            print('
			<div id="message" style="margin-bmp:30px" class="updated fade">
    ...
    print('
			<div id="help" style="overflow: hidden;padding-left:15px; font-size: 14px; margin-bmp: 25px; background-color: rgb(239, 239, 239);">
    ...
    print('
			<div id="help" style="overflow: hidden;padding-left:15px; font-size: 14px; margin-bmp: 25px;margin-bottom:25px; background-color: rgb(239, 239, 239);">
...
if($excludeList.length >0)
{
 ...
 print('<form id="bmp_TweetOldPost" name="bmp_TweetOldPost" action="' . get_bloginfo('wpurl') . '/wp-admin/admin.php?page=BMPExcludePosts" method="post"><input type="hidden" name="delids" id="delids" value="' . $exposts . '" /><input type="submit" id="pageit" name="pageit" style="display:none" value="" /> ');
        ...
        print('<select name="selFilter" id="selFilter" style="width:100px"><option value="all" '.bmp_opt_optionselected("all",$ex_filter).'> All </option><option value="excluded" '.bmp_opt_optionselected("excluded",$ex_filter).'> Excluded </option></select>');
...
jQuery(function() {
  jQuery(".page-numbers").click(function(e){
         jQuery("#bmp_TweetOldPost").attr("action",jQuery(this).attr("href"));
         e.preventDefault();
         jQuery("#pageit").click();
...
return list.join(",");
				...
				function checkedAll() {
					var ischecked=document.bmp_TweetOldPost.headchkbx.checked;
					var delids="";
                                        for (var i = 0; i < document.bmp_TweetOldPost.chkbx.length; i++) {
        				document.bmp_TweetOldPost.chkbx[i].checked = ischecked;
        				if(ischecked)
        					delids=delids+document.bmp_TweetOldPost.chkbx[i].value+",";
                                         }
                                         document.getElementById("delids").value=delids;
                                }
